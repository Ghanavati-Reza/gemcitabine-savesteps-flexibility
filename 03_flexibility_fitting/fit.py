...
not included among calculated potentials. At the first part of the code we 
...
def calc_params_vect(single_geom,bonds,angles,dihedrals, atoms_and_indices, M):
    ...
    params = [bonds_vectors,bonds_lengths,angles_theta,dihedrals_phi,dihedral_angles]
    return params
...
npotentials = nstrech_potential + nbend_potential + ntorsion_potential
...
angle_type = []
for i in range(nbend_potential):
    angle_type.append([])
...
if Flag_RD == 'True':
    ...
    n_strech_and_bend = nstrech_potential + nbend_potential
       ...
       for i in range(len(list_torsion_type[0])):
          if m in list_torsion_type[1][i]:
             if (list_torsion_type[0][i] == 'Rotatable') or (list_torsion_type[0][i] == 'CO_Rotatable'):
                index_torsion = n_strech_and_bend + c
    ...
    X_array_column_num = c + n_strech_and_bend
...
if Flag_RD == 'True':
    ...
    params_fit = glmnet_python.glmnet(x = X_array.copy(), y = forces_energies.copy(),family = 'gaussian',alpha = 1, nlambda = 100, thresh = 1e-7, cl = c1_final, penalty_factor = PF_final, weights = weights ,standardize = False, standardize_resp = False, lambda_min = np.array([1e-5]), intr = False)   
    ...
    print(b_final[(nstrech_potential+nbend_potential):])
    print(' ')
    ...
    TA_non_rot = len(b_final[(nstrech_potential+nbend_potential):]) - TA_rot - TA_linear
    ...
    if TA_linear > 0:
        ...
        lambda0_linear = 0
    lambda0_non_rot = np.count_nonzero(b_final_0[(nstrech_potential+nbend_potential):]) - lambda0_rot - lambda0_linear
    ...
    if TA_linear > 0:
        ...
        lambdaB_linear = 0
    lambdaB_non_rot = np.count_nonzero(b_final[(nstrech_potential+nbend_potential):]) - lambdaB_rot - lambdaB_linear
    ...
    print(str_table_K0); print(str_table_K1);print(str_table_K2); print(str_table_K3);print(str_table_K4); print(str_table_K5); print(str_table_K6); print(str_table_K7)
    print(' ')
    ...
    params_fit = glmnet_python.glmnet(x = X_array.copy(), y = forces.copy(),family = 'gaussian',alpha = 1, nlambda = 100, thresh = 1e-7, cl = c1_final, penalty_factor = PF_final, standardize = False, standardize_resp = False, lambda_min = np.array([1e-5]), intr = False)   
    ...
    TA_bends = len(b_final[nstrech_potential:(nstrech_potential+nbend_potential)])
    TA_linear = 2*sum(mode8_torsion_type)
    TA_non_rot = len(b_final[(nstrech_potential+nbend_potential):]) - TA_linear
    ...
    if TA_linear > 0:
        ...
        lambda0_linear = 0
    lambda0_non_rot = np.count_nonzero(b_final_0[(nstrech_potential+nbend_potential):]) - lambda0_linear
    ...
    if TA_linear > 0:
        ...
        lambdaB_linear = 0
    lambdaB_non_rot = np.count_nonzero(b_final[(nstrech_potential+nbend_potential):]) - lambdaB_linear
    ...
    print(str_table_K0); print(str_table_K1);print(str_table_K2); print(str_table_K3);print(str_table_K4); print(str_table_K5); print(str_table_K6);
    print(' ')
    str_table_K7 = '{:^35}  {:^15}  {:^15}  {:^15} \n'.format('Total',str(len(b_final)),str(np.count_nonzero(b_final_0)),str(np.count_nonzero(b_final)))
    print(str_table_K7)
