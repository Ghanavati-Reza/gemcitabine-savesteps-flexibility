...
def calc_params_vect(single_geom,bonds,angles,dihedrals, atoms_and_indices, M):
    ...
    for i in range(len(angles)):
        for j in range(len(angles[i])):
            ...
            # GROMACS-compatible bond-angle-cross redefinition: also compute
            # r13, the direct distance between the two OUTER (terminal)
            # atoms of this angle (angle_j[1] and angle_j[3]) -- not one of
            # the two flanking bonds, just a geometric distance. Same
            # closest-image treatment as the bond-vector computation above,
            # for consistency.
            atom13_A = angle_j[1]
            atom13_B = angle_j[3]
            R13_A = single_geom[atom13_A-1, :]
            R13_B = single_geom[atom13_B-1, :]
            R13_B_pos_indice = calc_closest_image_pos(R13_A, R13_B, M)
            R13_B_close = R13_B_pos_indice[0]
            r13_length = np.linalg.norm(R13_B_close - R13_A)
            angle_r13_length[i].append(r13_length)
    ...
    params = [bonds_vectors,bonds_lengths,angles_theta,dihedrals_phi,dihedral_angles,angle_bonds_length,angle_r13_length]
    return params
...
def calc_delta_bond_angle13_potential(affected_angles, angle_bond_params_SG, angle_bond_params_SGD, equil_angle_bond_params, r13_params_SG, r13_params_SGD, equil_r13_params, atom):
    # GROMACS-native form: U = k*(r13-r13eq)*[(r1-r1eq)+(r2-r2eq)] -- couples
    # to the direct 1-3 (outer-atom-to-outer-atom) distance instead of the
    # angle deviation used by the original calc_delta_bond_angle_potential.
    # Matches GROMACS's angle function type 4 ("cross bond-angle") exactly,
    # unlike the theta-coupled form (see docs/06's GROMACS-compatible
    # variant section for why this redefinition was needed). The math
    # structure is otherwise identical -- only the meaning of the third
    # coupled quantity (r13 here, theta in the original) changed.
    delta_bond_angle13_potential = np.zeros(shape = (len(equil_angle_bond_params), 1), dtype=float)
    for index, affected_angle in enumerate(affected_angles[atom-1]):
        i = affected_angle[0]
        j = affected_angle[1]
        ...
        r13_dev_SG = r13_params_SG[i][j] - equil_r13_params[i][j]
        bond_angle13_potential_SG_i = (r1_dev_SG + r2_dev_SG) * r13_dev_SG
        ...
        r13_dev_SGD = r13_params_SGD[i][j] - equil_r13_params[i][j]
        bond_angle13_potential_SGD_i = (r1_dev_SGD + r2_dev_SGD) * r13_dev_SGD

        delta_bond_angle13_potential[i] += (bond_angle13_potential_SG_i - bond_angle13_potential_SGD_i)*10000

    return delta_bond_angle13_potential
...
def calc_params_vect_displaced_atoms(affected_bonds, affected_angles, affected_dihedrals, bond_type, angle_type, dihedral_type, atom, displaced_geom, params_vect_single_geom, atoms_and_indices, M):
    ...
    for index, affected_angle in enumerate(affected_angles[atom-1]):
        ...
        # GROMACS-compatible bond-angle-cross redefinition: recompute r13
        # (outer-atom to outer-atom distance) from the displaced geometry
        # too, using the same angle_j[1]/angle_j[3] outer-atom convention
        # as calc_params_vect. This loop is already gated to angles
        # affected by displacing `atom` (whether atom is the vertex or
        # either outer atom), the same gating that's already correct for
        # theta/bonds above, so no additional gating logic is needed.
        R13_A_displaced = displaced_geom[angle_j[1]-1, :]
        R13_B_displaced = displaced_geom[angle_j[3]-1, :]
        R13_B_displaced_pos_indice = calc_closest_image_pos(R13_A_displaced, R13_B_displaced, M)
        R13_B_displaced_close = R13_B_displaced_pos_indice[0]
        r13_displaced = np.linalg.norm(R13_B_displaced_close - R13_A_displaced)
        params_vect_displaced[6][i][j] = r13_displaced
...
# GROMACS-compatible bond-angle-cross redefinition: r13 (outer-atom to
# outer-atom distance) equilibrium values, averaged per angle type the
# same way r1eq/r2eq/thetaeq are averaged below.
angles_r13_n = params_vect_equil[6]
...
for i in range(len(angles_theta_n)):
    ...
    avg_angle_r13_length = angle_r13_length_sum/len(angles_theta_n[i])
    ...
    for j in range(len(angles_theta_n[i])):
        ...
        angles_r13_avg[i].append(avg_angle_r13_length)
...
params_vect_equil[6] = angles_r13_avg
...
for geom_number in range(1,ngeoms_train+1):
    ...
    for atom in range(1,natoms+1): 
        for coord in range(3):
            ...
            bond_angle_potential = calc_delta_bond_angle13_potential(affected_angles,params_vect_single_geom[5],params_vect_displaced[5],params_vect_equil[5],params_vect_single_geom[6],params_vect_displaced[6],params_vect_equil[6],atom)
...
for geom_number_val in range(1,ngeoms_val+1):
    ...
    for atom in range(1,natoms+1):
        for coord in range(3):
            ...
            bond_angle_potential_val = calc_delta_bond_angle13_potential(affected_angles,params_vect_single_geom_val[5],params_vect_displaced_val[5],params_vect_equil[5],params_vect_single_geom_val[6],params_vect_displaced_val[6],params_vect_equil[6],atom)
